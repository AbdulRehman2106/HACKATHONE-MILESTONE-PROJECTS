import React from 'react'
import { FaFacebook, FaInstagram, FaTwitter } from "react-icons/fa"
import Image from 'next/image'


const Footer = () => {
  return (
    <footer className='bg-white py-4 font-montserrat'>

      <div className="max-w-[1440px] mx-auto px-6">

        <div className="flex flex-wrap items-center justify-between bg-gray-100 py-6 px-4 rounded-lg">

          <h3 className="text-xl font-extrabold text-gray-800">Bandage</h3>

          <div className="flex gap-4">
            
            <a href="#" className='text-blue-600 text-xl'><FaFacebook /></a>
            <a href="#" className='text-blue-600 text-xl'><FaInstagram /></a>
            <a href="#" className='text-blue-600 text-xl'><FaTwitter /></a>

            </div>
        </div>

        <div className="w-full h-px bg-gray-300 my-6"></div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8 mb-10 "> 

            <div>

                <h4 className="text-lg font-extrabold text-gray-800 mb-4">Company Info</h4>

                <ul className="space-y-2 text-gray-600 text-sm">

                  <li>About Us</li>
                  <li>Carrier</li>
                  <li>We Are Hiring</li>
                  <li>Blog</li>

                </ul>
            </div>

            <div>

                <h4 className="text-lg font-extrabold text-gray-800 mb-4">Legal</h4>
                
                <ul className="space-y-2 text-gray-600 text-sm">

                  <li>About Us</li>
                  <li>Carrier</li>
                  <li>We Are Hiring</li>
                  <li>Blog</li>

                </ul>
            </div>

            <div>

                <h4 className="text-lg font-extrabold text-gray-800 mb-4">Features</h4>
                
                <ul className="space-y-2 text-gray-600 text-sm">

                  <li>Bussneiss Marketing</li>
                  <li>User Analytics</li>
                  <li>Live Chat</li>
                  <li>Unlimited Support</li>

                </ul>
            </div>

            <div>

                <h4 className="text-lg font-extrabold text-gray-800 mb-4">Resource</h4>
                
                <ul className="space-y-2 text-gray-600 text-sm">

                  <li>IOS & Android</li>
                  <li>Watch A Demo</li>
                  <li>Customers</li>
                  <li>API</li>

                </ul>
            </div>

            <div>

              <h4 className="text-lg font-extrabold text-gray-800 mb-4">Get In Touch</h4>

              <form className="flex items-center gap-2">

                <input 
                type="email" 
                placeholder='Enter Your Email'
                className='w-full md:w-2/3 px-4 py-3 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500' />


              <button
              type='submit'
              className="bg-blue-600 text-white font-extrabold px-4 py-3 rounded-md hover:bg-blue-700 text-sm">Subscribe</button>
              </form>

              <p className="text-gray-600 text-sm mt-2 ">Lore Imp Sum Dolor Amit</p>

            </div>
      </div>
    </div>

    <div className="container mx-auto flex flex-col items-center space-y-4">

<h1 className="text-3xl font-extrabold font-serif text-blue-400 hover:text-cyan-400 "><i>Abdul Rehman</i></h1>

<div className="flex space-x-10">
    <a href="https://www.facebook.com/its.abdul.rehman.17?mibextid=ZbWKwL"
    target="_blank" 
    rel="noopener noreferrer"
    >
    <Image
        src="https://upload.wikimedia.org/wikipedia/commons/5/51/Facebook_f_logo_%282019%29.svg"
        alt="Facebook"            
        width={56}
        height={56}
        className="hover:opacity-75"
    />
    </a>
    
    <a href="https://www.linkedin.com/in/abdul-rehman-2213012b9?utm_source=share&utm_campaign=share_via&utm_content=profile&utm_medium=android_app"
        target="_blank"
        rel="noopener noreferrer"
    >
    <Image
        src="https://upload.wikimedia.org/wikipedia/commons/e/e9/Linkedin_icon.svg"
        alt="Linkedin"
        width={56}
        height={56}
        className="hover:opacity-75"
     />
    </a>

    <a href="https://github.com/AbdulRehman2106"
        target="_blank"
        rel="noopener noreferrer"
    >
    <Image
        src="data:image/png;base64,"
        alt="Github"
        width={56}
        height={56}
        className="hover:opacity-75"
     />
    </a>
</div>
</div>
</footer>
  )
}

export default Footer
